...
class E6
{
    ...
    //         None of those values are null and t is in ascending
    //         order.
    ...
    public static<T extends Comparable<? super T>> void searchRecursive( AVL<T> t, T x, Pair<T> p )
    {
        ...
        else if( AVL.rootValue(t).compareTo(x) ?hér vantar samanburð - missing comparator here? 0 )
        {
            ... hér vantar forritstexta -- missing code here
        }
        else
    ...
    {
        if( t == null ) return;
        if( AVL.rootValue(t).compareTo(x) ?samanburður - comparator? 0 )
        {
            ... hér vantar forritstexta -- missing code here
        }
        else
